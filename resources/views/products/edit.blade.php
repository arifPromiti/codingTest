                            ...
                            <div class="card shadow mb-4">
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="title" placeholder="Product Name" value="{{$product->title}}" class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label for="">Product SKU</label>
                                        <input type="text" name="sku" placeholder="Product SKU" value="{{$product->sku}}"class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label for="">Description</label>
                                        <textarea id="" name="description" cols="30" rows="4" class="form-control">{{$product->description}}</textarea>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <tbody id="preview">
                                                @if(isset($productVariantPrice))
                                                    @foreach($productVariantPrice as $item)
                                                        @php
                                                            $vpdata = $item->getproduct_variant_oneData->variant.'/'.$item->getproduct_variant_twoData->variant.'/'.$item->getproduct_variant_threeData->variant;
                                                            $vdata =  $item->getproduct_variant_oneData->variant_id.'/'.$item->getproduct_variant_twoData->variant_id.'/'.$item->getproduct_variant_threeData->variant_id;  
                                                        @endphp
                                                        <tr>
                                                            <td>
